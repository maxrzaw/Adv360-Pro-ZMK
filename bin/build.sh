...
# West Build (left)
west build -s zmk/app -p -d build/left -b adv360_left -S studio-rpc-usb-uart -S zmk-usb-logging -- -DZMK_CONFIG="${PWD}/config" -DCONFIG_ZMK_STUDIO=y
...
if [ "${BUILD_RIGHT}" = true ]; then
    # West Build (right)
    west build -s zmk/app -p -d build/right -b adv360_right -S zmk-usb-logging -- -DZMK_CONFIG="${PWD}/config"
